...
class Environment(object):
    ...
    def __init__(self,
                 ...
                 # array with 1 to 8 items, values from 1 to 8
                 enemies=[1],
    ...
    def load_sprites(self):
        ...
        self.player = Player((start_cell.px, start_cell.py),
                             self.enemyn, self.level, self.sprite_p)
    ...
    def state_to_log(self):
        ...
        self.print_logs(
            "# contact hurt (training agent) # : " + self.contacthurt)
    ...
    def save_state(self):
        ...
        file = gzip.open(self.experiment_name +
                         '/evoman_solstate', 'w', compresslevel=5)
    ...
    def checks_params(self):
        ...
        if self.loadplayer not in ('yes', 'no'):
            self.print_logs(
                "ERROR: 'load player' value must be 'yes' or 'no'.")
            sys.exit(0)
        ...
        if self.inputscoded not in ('yes', 'no'):
            self.print_logs(
                "ERROR: 'inputs coded' value must be 'yes' or 'no'.")
            sys.exit(0)

        if self.multiplemode not in ('yes', 'no'):
            self.print_logs(
                "ERROR: 'multiplemode' value must be 'yes' or 'no'.")
            sys.exit(0)
        ...
        if self.speed not in ('normal', 'fastest'):
            self.print_logs(
                "ERROR: 'speed' value must be 'normal' or 'fastest'.")
            sys.exit(0)
        ...
        if self.clockprec not in ('low', 'medium'):
            self.print_logs(
                "ERROR: 'clockprec' value must be 'low' or 'medium'.")
            sys.exit(0)
        ...
        if self.contacthurt not in ('player', 'enemy'):
            self.print_logs(
                "ERROR: 'contacthurt' value must be 'player' or 'enemy'.")
            sys.exit(0)
        ...
        if self.level < 1 or self.level > 3:
            self.print_logs(
                "MESSAGE: 'level' chosen is out of recommended (tested).")
    ...
    def run_single(self, enemyn, pcont, econt):
        ...
        while 1:
            ...
            pygame.draw.line(self.screen, (0,   0,   0),
                             [40, 45], [140, 45], 5)
            pygame.draw.line(self.screen, (150, 24, 25),   [
                             40, 45], [140 - vbar, 45], 5)
            pygame.draw.line(self.screen, (0,   0,   0),
                             [40, 49], [140, 49], 2)
            ...
            pygame.draw.line(self.screen, (0,   0,   0),
                             [590, 45], [695, 45], 5)
            pygame.draw.line(self.screen, (194, 118, 55),  [
                             590, 45], [695 - vbar, 45], 5)
            pygame.draw.line(self.screen, (0,   0,   0),
                             [590, 49], [695, 49], 2)
            ...
            def return_run():
                self.print_logs("RUN: run status: enemy: "+str(self.enemyn)+"; fitness: " + str(fitness) + "; player life: " +
                                str(self.player.life) + "; enemy life: " + str(self.enemy.life) + "; time: " + str(self.time))
            ...
            if self.start == False and self.playermode == "human":
                ...
                self.screen.blit(myfont.render(
                    "Player", 1,  (150, 24, 25)), (50, 180))
                self.screen.blit(myfont.render(
                    "  VS  ", 1,  (50, 24, 25)), (250, 180))
                self.screen.blit(myfont.render(
                    "Enemy "+str(self.enemyn), 1,  (194, 118, 55)), (400, 180))
            ...
            if self.player.life == 0:
                ...
                if self.playermode == "human":
                    ...
                    pygame.font.Font.set_bold
                    self.screen.blit(myfont.render(
                        " Enemy wins", 1, (194, 118, 55)), (150, 180))
            ...
            if self.enemy.life == 0:
                ...
                if self.playermode == "human":
                    ...
                    pygame.font.Font.set_bold
                    self.screen.blit(myfont.render(
                        " Player wins ", 1, (150, 24, 25)), (170, 180))
    ...
    def multiple(self, pcont, econt):
        ...
        for e in self.enemies:
            ...
            fitness, playerlife, enemylife, time = self.run_single(
                e, pcont, econt)
